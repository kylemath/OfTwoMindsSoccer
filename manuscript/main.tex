% ============================================================
% The Subspace Penalty Kick: How Shared Neural Representations
% Shape the Duel Between Goalkeeper and Striker
% ============================================================
...
% ============================================================
\title{\textbf{The Subspace Penalty Kick}\\[0.3em]
\large How Shared Neural Representations Create Exploitable\\
Interference in the Goalkeeper--Striker Duel}
...
I argue that both goalkeeper and striker execute their decisions by combining sensory categorisation and motor response subspaces that are \emph{shared} across different action plans.
...
I derive seven testable predictions and evaluate five of them against two public penalty kick datasets ($N = 482$ international penalties and $N = 6{,}392$ Bundesliga penalties from 1963--2025).
The most striking result concerns sequential effects: goals are significantly more likely after a previous goal than after a save ($\PredTwoGoalAfterGoal$ vs.\ $\PredTwoGoalAfterSave$; $\chi^2 = \PredTwoChiSq$, $p = \PredTwoChiP$), and goalkeepers repeat their dive direction $\PredTwoKaggleRepeatRate$ of the time across same-match penalties ($p = \PredTwoKaggleBinomP$, binomial test), consistent with the ``task-history ghost'' predicted by the subspace framework.
...
\textbf{Keywords:} penalty kick, shared neural subspaces, compositional cognition, motor interference, action selection, gain modulation, sports neuroscience
...
The result is a high-stakes perceptual-motor guessing game that has attracted analysis from economics \citep{palacioshuerta2003minimax}, psychology \citep{bareli2007action}, biomechanics \citep{diaz2012anticipating}, and perception science \citep{wood2015quiet}.
Game-theoretic analysis suggests professional kickers and goalkeepers approximate minimax equilibrium play, randomising across directions such that neither player can improve by unilaterally changing strategy \citep{palacioshuerta2003minimax}.
...
In this paper I propose that a recent discovery in systems neuroscience offers a surprisingly natural framework for understanding the penalty kick duel, and one that generates novel predictions.
...
I propose that both the goalkeeper and the striker face compositional task structures with shared subspaces, and that understanding the geometry of this sharing reveals why certain deception strategies work, why goalkeepers exhibit action bias, and why pre-commitment is advantageous for strikers.
...
This prediction runs counter to the gambler's fallacy (expecting alternation) but is consistent with the neural data showing persistent biases toward recent actions.

\begin{keeperbox}
\textbf{Tip:} After diving left, recognise that your brain maintains a ``ghost'' pull toward left. Use the walk-back to centre to mentally rehearse the \emph{opposite} direction as a corrective.
\end{keeperbox}

\begin{strikerbox}
\textbf{Tip:} Study the keeper's dive direction from \emph{two kicks ago}, not just the last kick. The neural ghost predicts repetition, so shooting to the same side the keeper dove two kicks prior may exploit this residual bias.
\end{strikerbox}
...
\begin{keeperbox}
\textbf{Tip:} Periodically train with deliberate attention to ``suppressed'' channels (gaze, shoulder angle) to prevent over-compression of potentially informative dimensions.
\end{keeperbox}

\begin{strikerbox}
\textbf{Tip:} Keep hips neutral as long as possible, then rotate late. The keeper's compressed gaze channel won't compensate for the high-gain hip channel's late arrival.
\end{strikerbox}

\subsection{Prediction 5: Keeper Centrality Maximises Striker Interference}

If the goalkeeper stays central and commits late, the striker must maintain both \task{Power Far Post} and \task{Placement Near Post} in parallel.
Because these share a motor subspace, the interference degrades both action plans simultaneously.
This provides a neural-level explanation for why goalkeepers who commit late perform better \citep{noelgomez2021interplay} and a mechanistic complement to the action bias paradox \citep{bareli2007action}: staying central may \emph{actively degrade the striker's motor execution} through shared-subspace interference.

\subsection{Prediction 6: Pre-Commitment Reduces Interference, Not Reaction Time}

Strikers who pre-decide where to shoot score more frequently than those who react to the goalkeeper \citep{dicks2010anticipation,noelgomez2021interplay}.
The standard explanation invokes reaction time limitations.

The subspace framework offers a different account: a pre-committed striker runs a single task with no shared-axis competition---analogous to C2, which uses a unique response axis and is executed accurately from trial~1.
A reactive striker runs the equivalent of C1 after an S1 block---maximum shared-axis interference.
The advantage of pre-commitment is not speed but \emph{motor purity} (Figure~\ref{fig:precommit}).
...
\begin{strikerbox}
\textbf{Tip:} Pick a side and commit. The neuroscience suggests this is not cowardice---it is motor hygiene.
\end{strikerbox}
...
I tested Predictions 1, 2, 4, 5, and 6 against two publicly available penalty kick datasets: an international-level dataset from Kaggle ($N = \PredOneNTotal$ penalties from 2020--2025 with shot direction, goalkeeper dive direction, and kicker foot) and the Bundesliga dataset ($N = \PredFourNTotal$ penalties from 1963--2017 with match context, goalkeeper identity, and outcome).
Predictions 3 and 7 require millisecond-level kinematic or neural data that these behavioural datasets cannot provide.
All analyses were conducted in Python; code, data, and a reproducible \texttt{Makefile} are available at the project repository.
...
\textbf{Results.} Goals were significantly more likely after a previous goal ($\PredTwoGoalAfterGoal$, $n = \PredTwoNAfterGoal$) than after a previous save ($\PredTwoGoalAfterSave$, $n = \PredTwoNAfterSave$; $\chi^2 = \PredTwoChiSq$, $p = \PredTwoChiP$).
In the Kaggle data, goalkeepers repeated their dive direction $\PredTwoKaggleRepeatRate$ of the time, significantly above the $33\%$ expected by chance ($p = \PredTwoKaggleBinomP$, binomial test; Figure~\ref{fig:emp_pred2}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/empirical_pred2_sequential.png}
\caption{\textbf{Prediction 2: sequential effects.} \textbf{A.} Bundesliga: goals are significantly more likely after a previous goal than after a save ($p = \PredTwoChiP$). \textbf{B.} Kaggle: goalkeepers repeated their dive direction $\PredTwoKaggleRepeatRate$ of the time in same-match sequences ($p = \PredTwoKaggleBinomP$).}
\label{fig:emp_pred2}
\end{figure}
...
The result also runs counter to the gambler's fallacy prediction (alternation) and to the minimax prediction of serial independence \citep{palacioshuerta2003minimax}.

However, alternative explanations exist.
The goal-after-goal effect may reflect within-match psychological momentum, a common penalty taker, or even selection bias (a team awarded multiple penalties may be dominant).
The goalkeeper repetition effect could reflect a strong side preference rather than a sequential bias.
The small sample of same-match pairs ($n = \PredTwoKaggleNSameMatch$) warrants caution.
Dedicated shootout data with sequential structure would provide a stronger test.
...
\midrule
Game Theory \citep{palacioshuerta2003minimax} & Players randomise to approximate minimax equilibrium & Serial independence may break down via task-history ghosts (P2; empirically supported) \\
\addlinespace
Action Bias \citep{bareli2007action} & Goalkeepers dive too often due to regret asymmetry & Staying central may also maximise striker motor interference (P5; underpowered test) \\
\addlinespace
...
\addlinespace
Quiet Eye \citep{wood2015quiet} & Deceptive gaze exploits attentional capture & Gaze deception exploits a \emph{suppressed} dimension, not an attended one \\
\addlinespace
Pre-commitment \citep{dicks2010anticipation} & Fixed-target strategy is better due to RT limits & Pre-commitment eliminates shared-subspace interference (P6; weak proxy) \\
\addlinespace
Point of No Return \citep{vanderkamp2018goalkeeping} & Discrete threshold for dive commitment & Continuous gradient of sensory$\rightarrow$motor transformation (P3; untested) \\
\bottomrule
\end{tabular}
\end{table*}
...
It is a compositional subspace war in which each player attempts to maximise the opponent's within-axis interference while minimising their own.
The goalkeeper wants to be a 50\% morph stimulus---maximally ambiguous---for as long as possible, and the striker wants to collapse their own task structure to a single, clean sensory-motor transformation.
...
The familiar advice---``pick a corner and commit''---may be not just motivational platitude but neural hygiene.
Pre-commitment eliminates shared-subspace interference.
The resulting motor programme is pure, fast, and uncontaminated by the ghost of a road not taken.
...
The penalty kick is simply where the stakes make that interference impossible to ignore.
...
No goalkeepers were harmed in the writing of this manuscript, though several hypothetical ones were thoroughly deceived.
This paper benefited from discussion with an AI assistant (Claude, Anthropic) in developing the initial framing, exploring the analogy space, and conducting preliminary analyses.
